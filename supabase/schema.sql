...
-- Bitácora privada para evitar recordatorios duplicados.
create table if not exists public.notification_log (
  id bigint generated by default as identity primary key,
  profile_id bigint not null references public.profiles(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  kind text not null check (kind in ('expiry_7d','expiry_3d','expiry_0d')),
...
drop policy if exists "No client access to notification log" on public.notification_log;
create policy "No client access to notification log"
